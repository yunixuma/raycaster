...
/*   Updated: 2023/02/24 21:42:24 by Yoshihiro K      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//debug_printf("pt(%3lf, %3lf)\n", pt->x, pt->y);
	dist = ft_math_distance_2d(&mlx->game.coord, pt) * cos(atan(ft_math_tan_deg(mlx->game.fov / 2) * i_col / (WIDTH_VISION >> 1)));
//debug_printf("dist: %3lf\n", dist);
	const_height = (WIDTH_VISION >> 1) / ft_math_tan_deg(mlx->game.fov / 2);
